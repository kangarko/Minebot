...
async def on_kick_create(event: hikari.AuditLogEntryCreateEvent) -> None:
    """
    Handles kick events from Discord audit logs, creating punishment entries
    and sending log messages as needed.
    """
    # --- Early validation checks ---
    # Skip if no target user or not a kick event
    if (target_id := event.entry.target_id) is None:
        return

    if event.entry.action_type != hikari.AuditLogEventType.MEMBER_KICK:
        return
    ...
    # Get the most recent kick for this user
    punishment = await PunishmentLogService.get_filtered_punishment_logs(
        user_id=target_id, punishment_type=PunishmentType.KICK, get_latest=True
    )
