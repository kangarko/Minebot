...
helper: CommandHelper = CommandHelper(CommandsKeys.UNLOCK)
...
class Unlock(
    ...
    name="extensions.unlock.label",
    description="extensions.unlock.description",
    default_member_permissions=helper.get_permissions(),
    hooks=helper.generate_hooks(),
    contexts=[hikari.ApplicationContextType.GUILD],
    localize=True,
):
    # Channel parameter allows moderators to specify which channel to unlock
    # If omitted, defaults to the current channel where command is executed
    channel: hikari.PartialChannel | None = lightbulb.channel(
        "extensions.unlock.options.channel.label",
        "extensions.unlock.options.channel.description",
    ...
    reason: str | None = lightbulb.string(
        "extensions.unlock.options.reason.label",
        "extensions.unlock.options.reason.description",
        default=None,
        localize=True,
    )
    ...
    async def invoke(self, ctx: lightbulb.Context) -> None:
        ...
        # Apply permission overwrite to restore message sending for @everyone role
        await channel.edit_overwrite(
            ...
            allow=hikari.Permissions.SEND_MESSAGES,  # Explicitly allow message sending
